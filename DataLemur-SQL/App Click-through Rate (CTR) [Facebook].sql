...
*/
SELECT app_id
     , ROUND(100.0 * click/impressions,2) ctr
FROM (SELECT app_id
           , SUM(CASE WHEN event_type = 'impression' THEN 1 END) impressions
           , SUM(CASE WHEN event_type = 'click' THEN 1 END) click
      FROM events
      WHERE (timestamp) >= '01/01/2022 00:00:00'
        AND (timestamp) <= '12/31/2022 00:00:00'
      GROUP BY 1)a