...
(SELECT merchant_id
      , credit_card_id
      , amount
      , transaction_timestamp prev_trans
      , LEAD(transaction_timestamp) OVER(PARTITION BY merchant_id, credit_card_id, amount ORDER BY transaction_timestamp) next_trans
FROM transactions)

SELECT COUNT(*) payment_cnt
FROM raw_data
WHERE next_trans IS NOT NULL
  AND DATE_PART('hour', next_trans - prev_trans)*60 + DATE_PART('minute', next_trans - prev_trans) <= 10