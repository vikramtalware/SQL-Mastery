...
WITH summary AS
(SELECT
   SUM(CASE WHEN item_type = 'prime_eligible' THEN square_footage END) prime_sq_ft
  ,COUNT(CASE WHEN item_type = 'prime_eligible' THEN item_id END) prime_items
  ,SUM(CASE WHEN item_type = 'not_prime' THEN square_footage END) non_prime_sq_ft
  ,COUNT(CASE WHEN item_type = 'not_prime' THEN item_id END) non_prime_items
FROM inventory)

,prime_items_area AS
(SELECT FLOOR(500000/prime_sq_ft)*prime_sq_ft max_prime_area
FROM summary)

SELECT 'prime_eligible' AS item_type
     , FLOOR(500000/prime_sq_ft)*prime_items item_count
FROM summary

UNION ALL

SELECT 'not_prime' AS item_type
     , FLOOR((500000-(SELECT max_prime_area FROM prime_items_area))/non_prime_sq_ft)*non_prime_items item_count
FROM summary;

/* Example Input -
-- prime_batch_Sq_Ft = 161.5
-- prime_batch_items = 3
-- prime_item_area = 500000/161.5 = 3095 these many batches will fit
-- one bactch contains 3 items hence 3095 batches will contain 3095*3 = 9285

-- non_prime_batch_Sq_Ft = 49
-- non_prime_batch_items = 2
-- non_prime_item_area = 500000 - (3095*161.5) = 157.5 = 157 available area for non_prime batches
--  157/49 = 3 batches will fit in that area
-- 3*2 = 6 items of non_prime items will fit*/