...
SELECT COALESCE(a.user_id,d.user_id) user_id
     , CASE WHEN (d.paid IS NULL and a.status IN ('NEW','EXISTING','CHURN','RESURRECT')) THEN 'CHURN' 
            WHEN (d.paid IS NOT NULL and a.status IN ('NEW','EXISTING','RESURRECT')) THEN 'EXISTING' 
            WHEN (d.paid IS NOT NULL and a.status IN ('CHURN')) THEN 'RESURRECT' 
            WHEN (d.paid IS NOT NULL and a.status IS NULL) THEN 'NEW' END new_status
FROM advertiser a
