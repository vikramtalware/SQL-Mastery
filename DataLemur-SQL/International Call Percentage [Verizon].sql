...
SELECT ROUND(100.0 * SUM(CASE WHEN caller_country != receiver_country THEN 1 END)/COUNT(*),1) intl_call_pct
FROM
(SELECT c.caller_id
      , p1.country_id caller_country
      , c.receiver_id
      , p2.country_id receiver_country
FROM phone_calls c
JOIN phone_info p1
  ON c.caller_id = p1.caller_id
JOIN phone_info p2
  ON c.receiver_id = p2.caller_id)a